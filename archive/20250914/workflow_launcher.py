...
def main():
    # 1) Load config (allow custom path as argv[1])
    cfg_path = Path(sys.argv[1]) if len(sys.argv) > 1 else Path("/Volumes/LaCie/_scripts/config.json")
    cfg = load_config(cfg_path)

    python_exec = cfg.get("python_exec", sys.executable or "python3")
    ...
    dailies_roll = Path(cfg.get("DEFAULT_DAILIES_ROLL", "/Volumes/Untitled/PRIVATE/M4ROOT/CLIP"))
