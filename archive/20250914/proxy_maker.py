...
def load_cfg() -> dict:
    cfg_path = os.getenv("CONFIG_PATH") or "/Volumes/LaCie/_scripts/config.json"
...
def main():
    ...
    media_pool_root = Path(cfg.get("MEDIA_POOL_ROOT", "/Volumes/LaCie/MEDIA_POOL"))
    proxy_pool_root = Path(cfg.get("PROXY_POOL_ROOT", "/Volumes/LaCie/PROXY_POOL"))
    ...
    for src in src_files:
        ...
        if out.exists() and not newer_than(src, out):
            continue
