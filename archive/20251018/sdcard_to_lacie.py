...
NAME = os.getenv("NAME")
if not NAME:
    die("NAME environment variable not set. This script must be run via workflow_launcher.py")

MEDIA_POOL_ROOT = Path(os.getenv("MEDIA_POOL_ROOT") or CFG.get("MEDIA_POOL_ROOT"))
if not MEDIA_POOL_ROOT:
    die("MEDIA_POOL_ROOT not found in config.json or environment variables")
...
suffix_pattern_str = CFG.get("suffix_allowed_pattern", r"^[A-Za-z0-9_-]+$")

BIN_PATTERN = re.compile(bin_pattern_str)
SUFFIX_ALLOWED = re.compile(suffix_pattern_str)  # what we allow as a suffix (no spaces)
