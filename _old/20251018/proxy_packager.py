...
def main():
    ...
    # 5) Create bucket and move candidates into it
    ...
    for src in candidates:
        ...
        try:
            ...
            print(f"Moved: {src.name} -> {dst.relative_to(sent_dir)}")
        except Exception as e:
            print(f"[WARN] Failed to move {src} -> {dst}: {e}")

    if not moved:
        print("[WARN] No folders were moved (unexpected).")
    else:
        print(f"\n✅ Packaged {len(moved)} folder(s) into: {bucket_path}")
